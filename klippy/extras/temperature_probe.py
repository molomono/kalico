...
#
# Merged from Klipper implementation:
# Copyright (C) 2024 Eric Callahan <arksine.code@gmail.com>
#
# For use with Kalico eddy current probes
...
class Polynomial2d:
    ...
    def __repr__(self):
        """Return human-readable polynomial representation"""
        parts = ["y(x) ="]
        deg = 2
    ...
    @classmethod
    def fit(cls, coords):
        """Fit a 2D polynomial to coordinate pairs (x, y)"""
...
class TemperatureProbe:
    """Temperature sensor with drift compensation for eddy current probes"""
    def __init__(self, config):
        self.name = config.get_name()
        self.printer = config.get_printer()
        self.gcode = self.printer.lookup_object("gcode")
        
        # Calibration and movement parameters
        ...
        # Register temperature sensor with heaters system
        ...
        # Register Drift Compensation Helper with probe if probe exists
        ...
        if config.has_section(full_probe_name):
            try:
                ...
                logging.warning(
                    "%s: failed to register drift compensation: %s"
                    % (self.name, str(e))
                )
        else:
            logging.info(
    ...
    def _temp_callback(self, read_time, temp):
        """Temperature callback - applies smoothing and tracks min/max"""
        ...
        # Check if we should advance to next calibration sample
    ...
    def _check_kick_next(self, eventtime):
        """Automatically advance calibration to next sample at target temperature"""
    ...
    def _prepare_next_sample(self, last_temp, tool_zero_z):
        """Prepare for next calibration sample"""
        # Register abort command for this step
        ...
        # Move tool down to resting position
        toolhead = self.printer.lookup_object("toolhead")
        cur_pos = toolhead.get_position()
    ...
    def _manual_probe_finalize(self, kin_pos):
        """Finalize manual probe step of calibration"""
        if kin_pos is None:
            # Calibration aborted
    ...
    def _get_probe(self):
        """Get the eddy current probe object"""
    ...
    def _check_homed(self):
        """Verify printer is homed before calibration"""
        toolhead = self.printer.lookup_object("toolhead")
    ...
    def _move_to_start(self):
        """Move to calibration start position"""
        toolhead = self.printer.lookup_object("toolhead")
        cur_pos = toolhead.get_position()
    ...
    def _get_speeds(self):
        """Get movement speeds from probe"""
    ...
    def cmd_TEMPERATURE_PROBE_CALIBRATE(self, gcmd):
        """Start temperature drift calibration"""
        ...
        expected_count = int((target_temp - cur_temp) / step + .5)
        ...
        try:
            ...
            self.gcode.register_command(
                ...
                desc=self.cmd_TEMPERATURE_PROBE_COMPLETE_help
            )
        ...
        # Move to heating position and turn on extruder if configured
        try:
            self._move_to_start()
    ...
    def cmd_TEMPERATURE_PROBE_NEXT(self, gcmd):
        """Move to next calibration sample location"""
        manual_probe.verify_no_manual_probe(self.printer)
        self.next_auto_temp = 99999999.
        toolhead = self.printer.lookup_object("toolhead")
        
        # Lift and move nozzle back to start position
    ...
    def cmd_TEMPERATURE_PROBE_COMPLETE(self, gcmd):
        """Complete calibration if enough samples collected"""
        manual_probe.verify_no_manual_probe(self.printer)
    ...
    def is_in_calibration(self):
        """Check if currently in calibration"""
...
class EddyDriftCompensation:
    """Manages temperature drift compensation for eddy current probes"""
    def __init__(self, config, sensor):
        ...
        # Load existing drift calibration if present
        dc = config.getlists(
    ...
    def is_enabled(self):
        """Check if drift compensation is enabled"""
        return self.enabled
    ...
    def note_z_calibration_start(self):
        """Called at start of Z calibration"""
        self.cal_temp = self.get_temperature()

    def note_z_calibration_finish(self):
        """Called at end of Z calibration - saves temperature"""
    ...
    def collect_sample(self, kin_pos, tool_zero_z, speeds):
        """Collect probe samples at multiple Z heights for calibration"""
        ...
        def _on_bulk_data_recd(msg):
            """Callback for receiving probe bulk data"""
            if move_times:
        ...
        # Collect samples at multiple Z heights
    ...
    def finish_calibration(self, success):
        """Finalize calibration and generate polynomials"""
        ...
        # Extract temperature range from calibration data
        min_temp, _ = cal_samples[0][0]
        max_temp, _ = cal_samples[-1][0]
        
        # Fit polynomials for each Z height
        polynomials = []
    ...
    def _check_calibration(self, calibration, start_temp, end_temp, error=None):
        """Validate that calibration polynomials are monotonic"""
    ...
    def adjust_freq(self, freq, origin_temp=None):
        """Adjust frequency from origin to calibration temperature"""
    ...
    def unadjust_freq(self, freq, dest_temp=None):
        """Adjust frequency from calibration to destination temperature"""
        if not self.enabled or freq < self.min_freq:
            return freq
    ...
    def _calc_freq(self, freq, origin_temp, dest_temp):
        """Calculate adjusted frequency based on temperature change"""
    ...
    def get_temperature(self):
        """Get current smoothed temperature from sensor"""
        return self.temp_sensor.get_temp()[0]


def load_config_prefix(config):
    """Load configuration and return TemperatureProbe object"""
